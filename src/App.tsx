...
const handleLogin = (e) => {
    ...
    if (pinInput === '4444') {
      ...
      foundUser = CARPENTERS.find(c => c.pin === pinInput) || { role: 'carpenter', name: 'Dave Mitchell', pin: '1234' };
      foundUser.role = 'carpenter';
    } else if (pinInput === '5678') {
      foundUser = CARPENTERS.find(c => c.pin === pinInput) || { role: 'carpenter', name: 'Ryan Cooper', pin: '5678' };
      foundUser.role = 'carpenter';
    } else if (pinInput === '1111') {
      foundUser = SITE_MANAGERS.find(s => s.pin === pinInput);
      foundUser.role = 'site_manager';
    } else if (pinInput === '2222') {
      foundUser = SITE_MANAGERS.find(s => s.pin === pinInput);
      foundUser.role = 'site_manager';
    } else if (pinInput === '9999') {
      foundUser = { role: 'invoice', name: 'Office/Invoice' };
    } else if (pinInput === '0000') {
      foundUser = { role: 'public', name: 'Guest' };
    }
...
{portal==="login"&&(<div style={{marginTop:64,minHeight:"90vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#0C1821"}}><div style={{background:"rgba(255,255,255,.03)",border:"1px solid rgba(255,255,255,.08)",borderRadius:12,padding:40,width:380,textAlign:"center"}}><div style={{width:60,height:60,background:"#B8860B",borderRadius:8,overflow:"hidden",margin:"0 auto 20px"}}><img src={logoUrl} alt="M&W" style={{width:"100%",height:"100%",objectFit:"contain"}}/></div><h2 style={{color:"#fff",fontSize:22,fontWeight:700,marginBottom:6}}>M&W Portal</h2><p style={{color:"rgba(255,255,255,.4)",fontSize:13,marginBottom:24}}>Enter your PIN to access the platform.</p><input type="password" value={pin} onChange={e=>setPin(e.target.value)} onKeyDown={e=>e.key==="Enter"&&doLogin()} placeholder="Enter PIN" style={{width:"100%",background:"rgba(255,255,255,.06)",border:"1px solid rgba(255,255,255,.1)",borderRadius:6,padding:"14px 16px",color:"#fff",fontSize:16,fontFamily:"inherit",outline:"none",textAlign:"center",letterSpacing:8,marginBottom:16}}/><button onClick={doLogin} style={{...S.bt,background:"#B8860B",color:"#fff",width:"100%",justifyContent:"center",padding:"14px"}}>Sign In</button><p style={{color:"rgba(255,255,255,.3)",fontSize:11,marginTop:16}}>Demo: 1234/5678 (Carpenter) | 9999 (Office) | 4444 (Admin)</p></div></div>)}
